...
export default function SearchBar({ onSearch }) {
  ...
  function submit(e){
    e.preventDefault()
    onSearch?.({ q, scope, stickers, include, exclude })
  }

  return (
    <form onSubmit={submit} className="bg-white border rounded-2xl p-4 grid md:grid-cols-5 gap-3">
      <input className="border rounded-lg px-3 py-2 col-span-2" placeholder="Search recipes…" value={q} onChange={e=>setQ(e.target.value)} />
      <select className="border rounded-lg px-3 py-2" value={scope} onChange={e=>setScope(e.target.value)}>
